...
const saveHomeHeroBannerLocal = async (filename: string, data: Buffer) => {
  const dir = join(getUploadsDir(), 'home-hero')
  await mkdir(dir, { recursive: true })
  await writeFile(join(dir, filename), data)
  return `/uploads/home-hero/${filename}`
}

const saveHomeHeroBannerS3 = async (filename: string, data: Buffer, contentType?: string) => {
  const { bucket } = getS3Config()
  const key = `home-hero/${filename}`
...
const saveHomePromoImageLocal = async (
  bannerId: string,
  breakpoint: string,
  filename: string,
  data: Buffer,
) => {
  const dir = join(getUploadsDir(), 'home-promos', bannerId)
  await mkdir(dir, { recursive: true })
  await writeFile(join(dir, `${breakpoint}-${filename}`), data)
  return `/uploads/home-promos/${bannerId}/${breakpoint}-${filename}`
}

const saveHomePromoImageS3 = async (
  bannerId: string,
  breakpoint: string,
  filename: string,
  data: Buffer,
  ...
  const { bucket } = getS3Config()
  const key = `home-promos/${bannerId}/${breakpoint}-${filename}`
...
export const saveHomePromoImage = async (
  bannerId: string,
  ...
  if (isS3StorageEnabled()) {
    return saveHomePromoImageS3(bannerId, breakpoint, filename, file.data, file.type)
  }

  return saveHomePromoImageLocal(bannerId, breakpoint, filename, file.data)
}
...
export const saveHomeHeroBanner = async (file: { data: Buffer, filename?: string, type?: string }) => {
  ...
  if (isS3StorageEnabled()) {
    return saveHomeHeroBannerS3(filename, file.data, file.type)
  }

  return saveHomeHeroBannerLocal(filename, file.data)
}
