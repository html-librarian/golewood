import { updateListingNewsSchema } from '#shared/schemas/listing-news'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = updateListingNewsSchema.safeParse(body)

  if (!parsed.success) {
    throw createError({ statusCode: 400, statusMessage: parsed.error.message })
  }

  return listingNewsService.update(newsId, listingId, user, parsed.data)
})
